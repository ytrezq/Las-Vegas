    ...
    for (ulong row1 = A; row1 < n; ++row1)
    {
        for (ulong row2 = B; row2 < n; ++row2)
        {
            ...
            if (isDependenceFound(result, mat_col, depdencyCol1, depdencyCol2))
            {
                ...
                double e_time = GetTime();
                cout << "\n Solved by processor Id ::" << processorId << "\t on node :: " << NodeName << " \t Time :: " << (e_time - s_time) << " seconds \n";
                cout << "\n GESC !ZM @pId :: " << processorId << "\t row1 :: " << row1 << "\t row2 :: "
                     << row2 << "\t col1 :: " << depdencyCol1 << "\t col2 :: " << depdencyCol2 << endl;

                {
                    // making a larger minor (i.e. mapping the two minor from hPrime to the original matrix)
                    ulong largerMinorDim = orgMat.NumRows() - mat.NumRows();
                    largerMinorDim += 2;
                    ulong I[largerMinorDim], J[largerMinorDim];

                    for (ulong i = 0; i < largerMinorDim - 2; ++i)
                        I[i] = J[i] = i;

                    I[largerMinorDim - 2] = row1 + (largerMinorDim - 2);
                    J[largerMinorDim - 2] = depdencyCol1 + (largerMinorDim - 2);

                    I[largerMinorDim - 1] = row2 + (largerMinorDim - 2);
                    J[largerMinorDim - 1] = depdencyCol2 + (largerMinorDim - 2);

                    for (int i = 0; i < largerMinorDim; ++i)
                        cout << I[i] << " ";
                    ...
                    _mat_ minor;
                    minor.SetDims(largerMinorDim, largerMinorDim);
