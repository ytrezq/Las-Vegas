...
bool schurComplement_OE(const _mat_ &mat, ulong obliqueCnt, ZZ ordP, const _mat_ orgMat)
{
    ...
    if (obliqueCnt > 1)
    {
        ...
        // cout << "\n row1_start :: " << row1_start << "\t col1_start :: " << col1_start << endl;
                ...
                for (ulong col1 = col1_start; col1 < row_; ++col1)
                {
                    for (ulong col2 = col1 + 1; col2 < row_; ++col2)
                    {
                        ...
                        if (IsZero(det))
                        {
                            ...
                            // cout << "\n twoM :: \n"
                            //      << twoM << endl;
                            // cout << "\n det(twoM) :: " << determinant(twoM) << endl;
                            // cout << "\n row1 :: " << row1 << "\t row2 :: " << row2 << "\t col1 :: " << col1 << "\t col2 :: " << col2 << endl;

                            {
                                ...
                                // if (IsZero(determinant(minor)))
                                {
                                    cout << "\n OESC - processorId (ker) :: " << processorId;
                                    cout << "\t obliqueCnt :: " << obliqueCnt;
                                    cout << "\t orgMat => det :: " << conv<FiniteField>(determinant(minor));
...
bool obliqueElimination(ZZ ordP)
{
    ...
    for (int i = 1; i < (matDim / 2); ++i)
    {
        ...
        if (schurComplement_OE<_mat_, FiniteField>(mat1, i, ordP, ker))
        {
            MPI_Abort(MPI_COMM_WORLD, 73);
            return true;
        }
